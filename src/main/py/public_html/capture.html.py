...
def get_picture_from_list_for_time(l_file_at_hour, l_hour, l_minute):
    # To avoid "Shadows name 'variable' from outer scope", local variables are prefixed with 'l_' for local
    for i in range(0, 14):
        l_hour_key = "{:02d}".format(l_hour) + "-" + "{:02d}".format(l_minute + i)
        if l_hour_key in l_file_at_hour:
            (l_image_name, image_size) = l_file_at_hour[str(l_hour_key)]
            if image_size > SIZE_LIMIT:
                l_background_color = "Khaki"
            else:
                l_background_color = "LightBlue"
            return str("{:02d}".format(l_minute + i)), l_image_name, str(l_background_color)
    return str("{:02d}".format(l_minute)), None, str("DarkGray")


def get_newest_file(l_folder):
    l_folder_scan = os.scandir(l_folder)
    max_mtime = -1
    last_file = ""
    has_daylight_image = False
    # TODO Find something better than below to get a "significative" image
    # (ideas: default hour "1000"/"-1" for any last ?
    size_limit = SIZE_LIMIT * 2  # Even if considered "day", an extra value (+50%) is added to get a significative image
    for l_file_obj in l_folder_scan:
        l_name = l_file_obj.name
        if os.path.isfile(l_folder + "/" + l_name):
            mtime = l_file_obj.stat().st_mtime
            log.debug("Stats from file '{0}': mtime = {1}".format(l_name, mtime))
            l_size = l_file_obj.stat().st_size
            if has_daylight_image:
                if mtime > max_mtime and l_size > size_limit:
                    log.debug("-> newer daylight file '{0}'".format(l_name))
                    last_file = l_name
                    max_mtime = mtime
            else:  # only having night images
                if mtime > max_mtime or l_size > size_limit:
                    # We got a newer file than the current one... day picture or night?
                    log.debug("-> newer file '{0}' (previous was night)".format(l_name))
                    last_file = l_name
                    max_mtime = mtime
                    if l_size > SIZE_LIMIT:
...
def get_newest_folder(l_folder):
    l_folder_scan = os.scandir(l_folder)
    max_mtime = -1
    last_folder = ""
    for l_file_obj in l_folder_scan:
        l_name = l_file_obj.name
        if os.path.isdir(l_folder + "/" + l_name):
            mtime = l_file_obj.stat().st_mtime
            if mtime > max_mtime:
                last_folder = l_name
                max_mtime = mtime
...
# "grangette/2021/01-10/grangette_2021-01-10Z16-00-07.jpg" or
# "grangette/2021/01-10/" or
# "grangette"
...
if image:
    ...
    if os.path.isfile(HOME + "/captures/" + image):  # <- camera / year / month_day / jpeg_image
        ...
        bottom_note = bottom_note + "/!\\ image " + str(month_day) + "/" + str(
            jpeg_image) + " given in parameter is not valid!<br/>"
        ...
        bottom_note = bottom_note + "/!\\ image " + str(year) + "/" + str(month_day) + "/" + str(
            jpeg_image) + " given in parameter is not valid!<br/>"
        ...
        bottom_note = bottom_note + "/!\\ image " + str(camera) + "/" + str(year) + "/" + str(month_day) + "/" + str(
            jpeg_image) + " given in parameter is not valid!<br/>"
...
html = html + """</table>
<img src=\"captures/""" + camera + "/" + year + "/" + month_day + "/" + str(jpeg_image) + \
 """\" style=\"width: 60em; height: 45em\" /><br/><br/>
